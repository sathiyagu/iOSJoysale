...
- (void)viewDidLoad {
    ...
    _contentScrollView.frame = CGRectMake(0,_topBarHeight + kYSLScrollMenuViewHeight, self.view.frame.size.width, self.view.frame.size.height - (_topBarHeight + kYSLScrollMenuViewHeight)+500);
    ...
    _menuView.itemfont = [UIFont fontWithName:appFontRegular size:15];
    _menuView.itemTitleColor = filterHeadercolor;
//    _menuView.itemIndicatorColor = self.menuIndicatorColor;
    _menuView.itemIndicatorColor = AppThemeColor;
...
{
    //NSLog(@"%lu",(unsigned long)self.childControllers.count);
    for (int i = 0; i < self.childControllers.count; i++)
    {
...
//    delegate.ContainerCurrentState = currentIndex;
}
