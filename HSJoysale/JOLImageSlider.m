...
- (id)initWithFrame:(CGRect)frame andSlides:(NSMutableArray *)slideSet
{
...
- (void) initialize {
    ...
    else{ _scrollView.scrollEnabled = NO; }
...
- (void) loadData
{
    if([_slideArray count] > 0)
    {
        ...
        NSDictionary * imageDetails = [_slideArray objectAtIndex: [_slideArray count]-1];
        ...
        [imageView sd_setImageWithURL:[NSURL URLWithString:[imageDetails objectForKey:@"bannerImage"]]];

//        [imageView setImageWithURL:[NSURL URLWithString:[imageDetails objectForKey:@"bannerImage"]] placeholderImage:[UIImage imageNamed:_placeholderImage]];
        ...
        for (int i = 0; i < [_slideArray count]; i++) {
            NSDictionary * imageDetails = [_slideArray objectAtIndex: i];
            
            CGRect imageFrame = CGRectMake(_scrollView.frame.size.width * (i+1)+18, 0, _scrollView.frame.size.width, _scrollView.frame.size.height);
...
//            [imageView setImageWithURL: [NSURL URLWithString:[imageDetails objectForKey:@"bannerImage"]] placeholderImage: [UIImage imageNamed:_placeholderImage]];
        ...
        //add first slide to the end
          ...
          imageDetails = [_slideArray objectAtIndex: 0];
        
        imageFrame = CGRectMake(_scrollView.frame.size.width * ([_slideArray count]+1)+18, 0, _scrollView.frame.size.width, _scrollView.frame.size.height);
