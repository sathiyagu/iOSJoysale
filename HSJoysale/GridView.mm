...
- (void)setup
{
    ...
    _sectionPadding = CGSizeMake(10.0,10.0);
    _cellSpacing = CGSizeMake(10.0,10.0);
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        _cellSize = CGSizeMake(240,275);
    }
    else
    {
        AppDelegate * theApp = (AppDelegate*)[[UIApplication sharedApplication]delegate];
        
        float widthheight = (theApp.windowWidth-40)/3;
        
            _cellSize = CGSizeMake(widthheight,widthheight);
...
{
    return [NSString stringWithFormat:@"%@ <section: %i, index: %u>", NSStringFromClass([self class]), (int)_section, (int)_index];
}
